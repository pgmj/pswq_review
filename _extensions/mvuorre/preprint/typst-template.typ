...
#let preprint(
  ...
  // List spacing
  show list: it => {
    // Space between list items (match body text leading)
    ...
    place(top, scope: "parent", float: true, {
      if title != none {
        align(center, block(width: 100%, above: 0em, below: 0em)[
          ...
          // Centred display lines: wrap at spaces and keep normal word
          // spacing, rather than hyphenating and stretching to full width.
        ...
        align(center)[
          #block(width: 100%, above: 1em, below: 0em)[
            #set par(justify: false)
            #text(weight: heading-weight, size: subtitle-size, hyphenate: false)[#subtitle]
          ]
        ]
      }
      ...
      if author_display != none {
        align(center)[
          #block(width: 100%, above: 2em, below: 0em)[
            #set par(justify: false)
            #text(weight: "regular", size: subtitle-size, hyphenate: false)[#author_display]
          ]
        ]
      }
      ...
      if affiliations != none {
        align(center)[
          #block(width: 100%, above: 1em, below: if date != none { 1em } else { 2em })[
            // hyphenate: false so long institution names wrap at spaces rather
            // than breaking mid-word ("Karolinska Insti-tutet"); justify: false
            // so the centred lines keep normal word spacing instead of being
            // stretched to the full page width
            #set par(justify: false)
            #text(weight: "regular", size: 1.1em, hyphenate: false)[
